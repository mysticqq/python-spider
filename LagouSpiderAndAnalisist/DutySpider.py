...
def save_to_mongo(info):
    try:
        if db['Descriptions'].insert(info):
            print('成功保存到数据库',info)
    except Exception:
        print('保存失败！',info)
...
def get_info_from_mongo():
    ...
    for r in res:
        businessZones.append(r['businessZones'])
        cities.append(r['cities'])
        companyFullName.append(r['companyFullName'])
        createTime.append(r['createTime'])
        education.append(r['education'])
        companyShortName.append(r['companyShortName'])
        jobNature.append(r['jobNature'])
        district.append(r['district'])
        companySize.append(r['companySize'])
        positionId.append(r['positionId'])
        positionName.append(r['positionName'])
        salary.append(r['salary'])
        workYear.append(r['workYear'])
    info = {
    ...
    df.to_csv('LagouPostion.csv',encoding='gb18030',index=False)
